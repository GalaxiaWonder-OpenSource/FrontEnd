...
export class MilestoneConfigurationComponent implements OnInit, OnDestroy {
  ...
  loadMilestones(): void {
    ...
    const milestonesSub = this.milestoneService.getByProjectId(projectId).subscribe({
  ...
  deleteMilestone(milestone: Milestone): void {
    if (confirm(this.translate.instant('milestone-configuration.confirm-delete'))) {
      this.loading = true;
      const milestoneId = typeof milestone.id === 'object' ? milestone.id : String(milestone.id);

      const deleteSub = this.milestoneService.delete(milestoneId).subscribe({
        next: () => {
          this.milestones = this.milestones.filter(m =>
            (typeof m.id === 'object' ? m.id : String(m.id)) !== milestoneId
          );
          this.loading = false;
          this.snackBar.open(
