...
export class TeamComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.projectId = this.sessionService.getProjectId() || '';
    this.orgId = this.sessionService.getOrganizationId() || '';
  ...
  async loadOrganizationMembers(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      const orgSub = this.orgMemberService.getByOrganizationId({ organizationId: this.orgId })
        .pipe(
          switchMap((members: OrganizationMember[]) => {
            if (members.length === 0) {
              this.orgMembers = [];
              return [];
            }
            
            const personIds = members.map((member: OrganizationMember) => member.personId.toString());
            const personRequests = personIds.map((id: string) => 
              this.personService.getById(null, { id })
            );
  ...
  submitAddMembers(): void {
    ...
    const addMemberRequests = membersArray.controls.map(control => {
      ...
      const newTeamMember = new ProjectTeamMember({
        role,
        specialty,
        memberId: new OrganizationMemberId(memberId),
        projectId: new ProjectId(this.projectId),
        // Asumimos que personId está disponible en el orgMember
        personId: new PersonId(this.getPersonIdFromOrgMember(orgMember))
      });
  ...
  getProjectRoleTranslation(role: string): string {
